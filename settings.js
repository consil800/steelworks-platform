...
// 설정 데이터 관리 (LocalDropdownSettings 사용)
const DropdownSettings = {
    // 설정 가져오기
    get: function() {
        return LocalDropdownSettings.get();
    },

    // 설정 저장하기
    save: function(settings) {
        LocalDropdownSettings.save(settings);
    },

    // 기본값으로 초기화
    reset: function() {
        localStorage.removeItem('dropdownSettings');
        return LocalDropdownSettings.defaultSettings;
    }
};
...
function addPaymentTerm() {
    ...
    if (settings.paymentTerms.includes(newTerm)) {
        Utils.showError('이미 존재하는 결제조건입니다.');
        return;
    }
    ...
    displayPaymentTerms(settings.paymentTerms);
    input.value = '';
    Utils.showSuccess('결제조건이 추가되었습니다.');
}
...
function addVisitPurpose() {
    ...
    if (settings.visitPurposes.includes(newPurpose)) {
        Utils.showError('이미 존재하는 방문목적입니다.');
        return;
    }
    ...
    displayVisitPurposes(settings.visitPurposes);
    input.value = '';
    Utils.showSuccess('방문목적이 추가되었습니다.');
}
...
function addColor() {
    ...
    if (settings.colors.some(color => color.name === newName)) {
        Utils.showError('이미 존재하는 색상 이름입니다.');
        return;
    }
...
function savePaymentTerm(index) {
    ...
    DropdownSettings.save(settings);
    displayPaymentTerms(settings.paymentTerms);
    Utils.showSuccess('결제조건이 수정되었습니다.');
}
...
function saveVisitPurpose(index) {
    ...
    DropdownSettings.save(settings);
    displayVisitPurposes(settings.visitPurposes);
    Utils.showSuccess('방문목적이 수정되었습니다.');
}
...
function deletePaymentTerm(index) {
    ...
    DropdownSettings.save(settings);
    displayPaymentTerms(settings.paymentTerms);
    Utils.showSuccess('결제조건이 삭제되었습니다.');
}
...
function deleteVisitPurpose(index) {
    ...
    DropdownSettings.save(settings);
    displayVisitPurposes(settings.visitPurposes);
    Utils.showSuccess('방문목적이 삭제되었습니다.');
}
...
function resetToDefaults() {
    ...
    loadSettings();
    Utils.showSuccess('모든 설정이 기본값으로 초기화되었습니다.');
}
